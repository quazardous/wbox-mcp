...
def _build_mcp_entry(cfg: dict, config_path: Path) -> dict:
    ...
    Generates a run.sh wrapper script because some MCP clients (e.g. Claude Code)
    do not reliably pass all args array elements to the subprocess.
    The wrapper embeds the absolute config path directly.
    """
    run_sh = _ensure_run_sh(config_path)
    return {
        "type": "stdio",
...
def _ensure_run_sh(config_path: Path) -> Path:
    """Create or update run.sh wrapper next to config.yaml.

    The wrapper invokes wbox-mcp with the absolute config path baked in,
    working around MCP clients that drop args.
    """
    command, _ = _detect_wbox_command()
    abs_config = str(config_path.resolve())
    run_sh = config_path.parent / "run.sh"

    content = f"""#!/usr/bin/env bash
# Auto-generated by wboxr — do not edit manually.
# Re-run 'wboxr register' to regenerate.
exec {command} serve "{abs_config}"
"""
